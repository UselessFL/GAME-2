...
async function StartOfTheGame(){
       ...
       function Game(){
        ...
        if(scene == 2){
            ...
            if(raund > 2){
             ...
             if(removableToyMass&&randomOfthree) 
              { for (let i = 0; i < 4; i++){ 
            ...
            removableToyMass[0] = toyToRemove;
   ...
   /*  scene = 1; */
    Game()
       ...
       columns[0] = 430
        ...
        function interactiveToys(bunny, win){
            ...
            function stopRotationOnHover() {
             ...
             win? bunny.on('pointerdown', ()=>{bunny.eventMode = 'none', stopRotationOnHover()}) : bunny.on('pointerdown', ()=>{bunny.eventMode = 'none',stopRotationOnHover()});
       ...
       async function drawToys(spriteNumber, scale, place, father, interactive, win){
        ...
        async function animationOnEnteringTheScene(){
            ...
            container.x += 500
            drawNumbers()
            AnimationOnButtonDown(true,2400)
            setTimeout(() => {
       ...
       function drawButton(TextToPut, color, parent, pos, endbutton){
        ...
        function AnimationOnButtonDown(scene, time){
            ...
            containerForFish.visible = false;
            containerForArrow.visible = false;
            containerOneForButton.visible= false;
            BackgroundStuff(2)
            setTimeout(() => {
                backk.tint = 0xFFFFFF
            ...
            containerForFish.visible = true;
            containerForArrow.visible = true;
            }, time?1500 + time: 1000);
        }
        ...
        anim.onFrameChange = (()=>{
            if(anim.currentFrame == 1){
                anim.stop()
                gsap.to(anim, {x: 253, duration: 2, onComplete(){anim.play()}})
            }
        if(anim.currentFrame == 2){
            anim.stop()
            gsap.to(anim, {x: 260, duration: 0, onComplete(){anim.play()}})
        }
        if(anim.currentFrame == 3){
            gsap.to(anim,{x:253, duration:0})
            anim.stop()
            gsap.to(anim, {x: 225, duration: 2, onComplete(){anim.play()}})
        }
        
        if(anim.currentFrame == 4){
            anim.stop()
            gsap.to(anim, {x: 220, duration: 0, onComplete(){anim.play()}})
        }
    ...
    const UIcontainer = new PIXI.Container();
     ...
     async  function drawUI(TextToPut,XX,width,gap, CrossNede, circleNeed){
       ...
       function drawCross(x,path,a){
        const cross = new PIXI.Sprite(PIXI.Texture.from(path) )
                cross.angle = 45
                cross.width = 25
                cross.height = 25
                a?cross.x = UI.x-UI.width/2 -25 + 25*x:cross.x = UI.x-UI.width/2 +25 - 25*x;
                cross.y = UI.y-UI.height/2-17;
                UIcontainer.addChild(cross)
       }
       function drawCircle(x, path, a){
        const WhiteCircle = new PIXI.Sprite(PIXI.Texture.from(path) )
        WhiteCircle.width = 20
        WhiteCircle.height = 20
       a?WhiteCircle.x = UI.x-UI.width/2 -10 + 25*x: WhiteCircle.x = UI.x-UI.width/2 +40 - 25*x;
        WhiteCircle.y = UI.y-UI.height/2-10;
        UIcontainer.addChild(WhiteCircle)
       }
        if(CrossNede){
            for(let x=0; x<3-OshibkiVIgre; x++){
               drawCross(x,'./Crosses/WhiteCross.png');
            }
            for(let x=0; x<OshibkiVIgre&& x<3; x++){
                drawCross(x, './Crosses/RedCross.png', true)
            }
        ...
        if(circleNeed){
            for(let x=0; x<3-TruesInARow; x++){
               drawCircle(x,'./Circles/YellowCircle.png')
            }
            for(let x=0; x<TruesInARow && x<3; x++){
               drawCircle(x, './Circles/WhiteCircle.png',true)
            }
    ...
    const containerForArrow = new PIXI.Container();
    containerForArrow.x =container.x
    containerForArrow.y = container.y
    container.addChild(containerForArrow)
    arrowAnimation();

    function arrowAnimation(){
        const arrow = new PIXI.Sprite(PIXI.Texture.from('./spritesTWO/arrow.png')  )
        arrow.anchor.set(0.5)
        arrow.scale.set(0.6)
        arrow.tint = 0x478fcc
        setPlace(arrow, -34, -445)
        arrow.visible=true
        gsap.to(arrow, {rotation: 2*3.14, duration: 60, ease:'steps(36)', repeat:-1, yoyo:false})
        function setPlace(name, X, Y){
            name.x = X
            name.y = Y
        }
        containerForArrow.addChild(arrow)
    }
    ...
    function BackgroundStuff(stateOfBack){
        ...
        if(stateOfBack==1){
            ...
            containerForArrow.tint=0x478fcc
        }
        if(stateOfBack==2){
           ...
           containerForArrow.tint = 0x2e648c;
